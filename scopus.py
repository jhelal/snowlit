...
def backward_snowballing(papers_df, output_csv_file):
    """
    ...
    for index, doi in enumerate(doi_list, start=1):
        ...
        try:
            ...
            for reference in ab.references:
                ...
                reference_series[
                    "Origin_DOI"
                ] = doi  # add the original DOI to the series
                reference_series[
                    "Origin_Index"
                ] = index  # add the document index to the series
                references_df = pd.concat(
                    [references_df, reference_series.to_frame().T], ignore_index=True
                )
...
def forward_snowballing(papers_df, output_csv_path):
    # Load the original document data from the CSV file
    original_documents = papers_df
    ...
    for i, original_document_row in original_documents.iterrows():
        ...
        for j, citing_doc_eid in enumerate(citing_docs_eids):
            ...
            if (
                citing_doc_eid in processed_eids
                or citing_doc_eid in original_document_eids
            ):
                continue
