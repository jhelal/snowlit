...
def read_csv_files(directory, keyword_filepath):
    ...
    keyword_dict = keyword_df.set_index('Filename')['Keyword String'].to_dict()
...
def compare_total_studies(data):
    ...
    save_plot_as_image(plt, image_name)
...
def compare_cumulative_documents_by_year(data):
    ...
    # Create a legend showing query names and keyword strings
    plt.legend(bbox_to_anchor=(0.5, -0.15), loc='upper center', ncol=1, fontsize='small')

    # Adjust the plot layout to make sure nothing is cropped
    plt.subplots_adjust(bottom=0.3)

    # Save the figure to a PNG file
    save_plot_as_image(plt, 'cumulative_documents_by_year_comparison.png')
...
def compare_top_sources(data, N):
    ...
    plt.legend(handles, labels, title_fontsize='13', bbox_to_anchor=(-0.18, -0.15), loc='upper left', ncol=1,
    ...
    # Save the figure to a PNG file
    save_plot_as_image(plt, 'top_sources_comparison.png')
...
def compare_top_authors(data, N):
    ...
    for csv_path, df_keyword in data.items():
        ...
        authors = df['Authors'].str.split(', ', expand=True).stack().str.strip()
        author_ids = df['Author(s) ID'].str.split(';', expand=True).stack().str.strip()
    ...
    handles, labels = plt.gca().get_legend_handles_labels()
    labels = [f"{label} ({data[label][1]})" for label in labels]  # access the keyword string from the tuple
    plt.legend(handles, labels, title_fontsize='13', bbox_to_anchor=(0.05, -0.15), loc='upper left', ncol=1,
               borderaxespad=0., fontsize='small')
    ...
    # Save the figure to a PNG file
    save_plot_as_image(plt, 'top_authors_comparison.png')
...
def list_additional_studies(data, original_data_name):
    ...
    for query, df_tuple in data.items():
        ...
        # Only keep the columns 'Authors', 'Year', 'Title', 'Source title', 'Cited by', 'DOI', 'Link', 'Abstract', 'Index Keywords'
        additional_df = additional_df[
            ['Authors', 'Year', 'Title', 'Source title', 'Cited by', 'DOI', 'Link', 'Abstract', 'Index Keywords']]

        # Convert 'Cited By' column to numeric (it might be stored as text) and sort in descending order
        additional_df['Cited by'] = pd.to_numeric(additional_df['Cited by'], errors='coerce').fillna(0)
        additional_df.sort_values('Cited by', ascending=False, inplace=True)

        # Add to the dictionary
        additional_studies[query] = additional_df

    output_file = r'C:\Users\james\PycharmProjects\lcareview\assets\additional_studies.xlsx'
